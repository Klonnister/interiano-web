...
// computed properties 
const dropdownPlaceholder = computed(() => createStore.loading ? 'Cargando...' : 'Seleccionar...')

const submit = async(event: Event) => {
  event.preventDefault();
  if(!createStore.validateView(1)) return false;
  if(!createStore.validateView(2)) return false;
  if(!createStore.validateView(3)) return false;
